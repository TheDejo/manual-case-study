...
export async function GET() {
  try {
    return NextResponse.json({data: questionnaireData, success: true, message: 'Questionnaire data fetched successfully'}, { status: 200 });
  } catch (error) {
    return NextResponse.json(
      { error: 'Failed to fetch questionnaire data', success: false, message: 'Failed to fetch questionnaire data' },
      { status: 500 }
    );
  }
}